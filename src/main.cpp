...
int main(int argc, char *argv[]) {
    ...
    // ------------------------------------
    string
        colormapShaderPath = string("../shaders/colormap-shaders-master/shaders/glsl/") + colorMap[0] + string(".frag");
    ...
    glTexImage2D(GL_TEXTURE_2D,
                 0,
                 GL_R32F,
                 resolution[1] * resScale,
                 resolution[0] * resScale,
                 0,
                 GL_RED,
                 GL_FLOAT,
                 NULL);
    ...
    if (strcmp(argv[1], "0") == 0) {
        ...
        while (!glfwWindowShouldClose(window)) {
            ...
            unsigned error =
                lodepng::encode((image_path[0] + "image" + std::to_string(imageCount)).c_str(), png, width, height);
...
void getZoomRange(float zoom_range[]) {
    ...
    input_check("Enter initial and final zoom level where zoom = 10^(level) (e.g. start_zoom end_zoom): ",
                2,
                zoom_range);
}
